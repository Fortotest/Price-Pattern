...
const PropertiesPanel = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-card text-foreground overflow-hidden w-full lg:w-[280px]">
      <div className="p-3 border-b border-border flex items-center justify-between bg-black">
        ...
        <Button aria-label="Close Properties Panel" variant="ghost" size="icon" onClick={onClose} className="h-6 w-6 hover:bg-muted flex lg:hidden">
          <X className="w-3 h-3" />
        </Button>
      </div>
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-6 pb-12">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground font-medium">Viewport Scale</span>
                <span className="text-[10px] font-mono text-primary">{(settings.zoom || 0.8).toFixed(2)}x</span>
              </div>
              <Slider 
                aria-label="Set Viewport Scale"
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground font-medium">Horizontal Density</span>
                <span className="text-[10px] font-mono text-primary">{(settings.spacing || 1.2).toFixed(2)}x</span>
              </div>
              <Slider 
                aria-label="Set Horizontal Density"
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground font-medium">Sequence Duration</span>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 gap-2">
                <div className="flex items-center justify-between gap-3 bg-black p-2 rounded-lg border border-border focus-within:border-primary/50 transition-colors">
                  <div className="flex items-center gap-2">
                    <Popover>
                      <PopoverTrigger asChild>
                        <button 
                          aria-label="Select Bullish Color"
                          className="w-6 h-6 rounded border border-border bg-muted cursor-pointer shadow-sm transition-transform active:scale-95" 
                          style={{ backgroundColor: settings.bullColor }}
                        />
                      </PopoverTrigger>
                      <PopoverContent className="w-fit p-3 bg-black border border-white/10 shadow-2xl rounded-md" side="right" align="center">
                        <HexColorPicker 
                          color={settings.bullColor} 
                          onChange={(color) => updateSettings({ bullColor: color })} 
                        />
                      </PopoverContent>
                    </Popover>
                    <span className="text-[9px] font-bold text-primary uppercase tracking-tight">Bullish</span>
                  </div>
                  <input 
                    aria-label="Input Bullish Hex Color"
                    type="text"
                    value={settings.bullColor}
                    onChange={(e) => updateSettings({ bullColor: e.target.value })}
                    className="bg-transparent border-none text-[10px] font-mono text-muted-foreground w-20 text-right focus:ring-0 outline-none p-0 uppercase"
                  />
                </div>
                ...
                <div className="flex items-center justify-between gap-3 bg-black p-2 rounded-lg border border-border focus-within:border-primary/50 transition-colors">
                  <div className="flex items-center gap-2">
                    <Popover>
                      <PopoverTrigger asChild>
                        <button 
                          aria-label="Select Bearish Color"
                          className="w-6 h-6 rounded border border-border bg-muted cursor-pointer shadow-sm transition-transform active:scale-95" 
                          style={{ backgroundColor: settings.bearColor }}
                        />
                      </PopoverTrigger>
                      <PopoverContent className="w-fit p-3 bg-black border border-white/10 shadow-2xl rounded-md" side="right" align="center">
                        <HexColorPicker 
                          color={settings.bearColor} 
                          onChange={(color) => updateSettings({ bearColor: color })} 
                        />
                      </PopoverContent>
                    </Popover>
                    <span className="text-[9px] font-bold text-destructive uppercase tracking-tight">Bearish</span>
                  </div>
                  <input 
                    aria-label="Input Bearish Hex Color"
                    type="text"
            ...
            <div className="space-y-3 pt-2">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground font-medium">Body Radius</span>
                <span className="text-[10px] font-mono text-primary">{settings.bodyRadius}px</span>
              </div>
              <Slider 
                aria-label="Set Body Corner Radius"
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-muted-foreground font-medium">Wick Radius</span>
                <span className="text-[10px] font-mono text-primary">{settings.wickRadius}px</span>
              </div>
              <Slider 
                aria-label="Set Wick Corner Radius"
...
const LayersPanel = ({ candles, onAddCandle, onUpdateCandle, onRemoveCandle, onClearAll, onTemplateLoad, onClose }: LayersPanelProps) => (
  <div className="flex flex-col h-full bg-card text-foreground overflow-hidden w-full">
    <div className="p-3 border-b border-border flex items-center justify-between bg-black">
      <div className="flex items-center gap-2">
        <Layers className="w-3.5 h-3.5 text-primary" />
        <span className="text-[10px] font-bold uppercase tracking-wider">Layer Stack</span>
      </div>
      <Button aria-label="Close Layers Panel" variant="ghost" size="icon" onClick={onClose} className="h-6 w-6 hover:bg-muted flex lg:hidden">
        <X className="w-3 h-3" />
      </Button>
    </div>
    
    <div className="p-3 bg-black space-y-4">
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <Label className="text-[9px] font-bold uppercase text-muted-foreground tracking-widest">Market Templates</Label>
          <Button 
            aria-label="Clear All Candles"
            variant="ghost" 
            size="sm" 
            onClick={onClearAll} 
            disabled={candles.length === 0}
            className="h-5 px-1.5 text-[8px] font-bold hover:bg-destructive/20 hover:text-destructive text-muted-foreground gap-1"
          >
            <Trash2 className="w-2.5 h-2.5" /> CLEAR ALL
          </Button>
        </div>
        <select 
          aria-label="Select Market Template"
          onChange={(e) => onTemplateLoad(e.target.value)} 
          className="flex w-full items-center justify-between rounded-md border border-border h-10 text-[10px] bg-black font-bold p-1 px-2 focus:ring-0 text-foreground outline-none"
        >
          <option value="custom">Clear Layers</option>
          ...
          <optgroup label="Macro Market Structures">
            ...
            <option value="bearish_snr_3_valleys">📉 SNR Bearish (3 Valleys)</option>
          </optgroup>
      ...
      <div className="grid grid-cols-3 gap-1.5">
        <Button aria-label="Add Bullish Candle" onClick={() => onAddCandle('Bullish')} className="bg-[#00ff9d] hover:bg-[#00ff9d]/90 h-7 text-[8px] font-bold border-none uppercase text-black">Bullish</Button>
        <Button aria-label="Add Bearish Candle" onClick={() => onAddCandle('Bearish')} variant="destructive" className="bg-[#ff3b30] hover:bg-[#ff3b30]/90 h-7 text-[8px] font-bold border-none uppercase">Bearish</Button>
        <Button aria-label="Add Doji Candle" onClick={() => onAddCandle('Doji')} variant="outline" className="bg-black hover:bg-muted/80 h-7 text-[8px] font-bold border-none uppercase text-foreground">Doji</Button>
      </div>
    </div>
    ...
    <ScrollArea className="flex-1">
      <div className="p-2 pb-8">
        <ManualEditor 
          candles={candles} 
          onChange={(idx, candle) => onUpdateCandle(idx, candle)} 
          onRemove={(idx) => onRemoveCandle(idx)} 
        />
      </div>
    </ScrollArea>
  </div>
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-black">
      {candles.length === 0 ? (
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full p-1 opacity-90" aria-label="Candlestick Preview">
          {candles.map((c, i) => {
...
export default function PricePatternEditor() {
  ...
  const playNotifSound = useCallback(() => {
    const audio = new Audio("https://raw.githubusercontent.com/Fortotest/Market.ai/055e319fa2c7c1e028057e8c69556c303b827848/hidup-jokowi.mp3");
    audio.volume = 1.0; 
  ...
  const handleExportSVG = useCallback(() => {
    ...
    showNotification("Successfully downloaded", "🥳");
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden font-body select-none text-foreground relative" onClick={unlockAudio}>
      <aside className={cn("flex-col flex-shrink-0 bg-card border-r border-border transition-all duration-300 ease-in-out lg:flex z-30", showProperties ? "w-[280px]" : "w-0 overflow-hidden border-none")}>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden transition-all duration-300 z-10">
        <header className="h-12 flex items-center justify-between px-4 border-b border-border bg-card z-30">
            ...
            <Sheet open={isConfigOpen} onOpenChange={setIsConfigOpen}>
              <SheetTrigger asChild>
                <Button 
                  ...
                  variant="ghost" 
                  size="icon" 
                  className="lg:hidden text-foreground h-9 w-9 hover:bg-muted"
                >
                  <Settings2 className="w-5 h-5" />
                </Button>
              </SheetTrigger>
              <SheetContent side="left" className="p-0 w-[280px] bg-card border-r border-border [&>button]:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet open={isLayersOpen} onOpenChange={setIsLayersOpen}>
              ...
              <SheetContent side="right" className="p-0 w-[280px] bg-card border-l border-border [&>button]:hidden">
            ...
            <a 
              aria-label="Visit masffadil Instagram" 
              href="https://www.instagram.com/masffadil/" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black hover:bg-muted border border-border group transition-all"
            >
              <Instagram className="w-3.5 h-3.5 text-pink-400 group-hover:scale-110 transition-transform" />
              <span className="text-[10px] font-bold tracking-wider text-foreground group-hover:text-primary hidden sm:inline">masffadil</span>
            </a>
          </div>
        </header>

        <div className="h-[105px] bg-black border-b border-border flex flex-col z-20 shrink-0 min-h-[105px]">
            ...
            <div className="flex items-center gap-4 py-3 min-w-max">
              {pages.map((page, idx) => (
                <div 
                  key={page.id} 
                  className={cn(
                    "group relative flex flex-col items-center gap-1.5 cursor-pointer transition-all flex-shrink-0",
                    activePageIndex === idx ? "opacity-100 scale-100" : "opacity-50 hover:opacity-100 scale-95 hover:scale-100"
                  )}
                  onClick={() => setActivePageIndex(idx)}
                >
                  <div className={cn(
                    "w-28 h-16 shrink-0 rounded-lg border-2 flex flex-col items-center justify-center bg-black overflow-hidden transition-all shadow-xl relative",
                    activePageIndex === idx ? "border-primary ring-4 ring-primary/20" : "border-border hover:border-muted-foreground/30"
                  )}>
                    <PagePreview candles={page.candles} settings={settings} />
                    
                    <div className="absolute bottom-1.5 left-2 z-10 pointer-events-none">
                      <span className="text-[12px] font-black text-foreground uppercase tracking-tighter drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)] dark:drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
                        {idx + 1}
                      </span>
                    </div>
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button 
                            ...
                            variant="ghost" 
                            size="icon" 
                            className="h-5 w-8 rounded-full bg-transparent hover:bg-primary data-[state=open]:bg-primary text-foreground transition-all"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <MoreHorizontal className="w-3 h-3" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="bg-popover border-border text-foreground min-w-[120px]">
                          <DropdownMenuItem 
                            aria-label="Duplicate Page"
                            onClick={(e) => { e.stopPropagation(); handleDuplicatePage(idx); }}
                            className="text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 focus:bg-muted focus:text-primary cursor-pointer"
                          >
                          ...
                          {pages.length > 1 && (
                            <DropdownMenuItem 
                              ...
                              className="text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 text-destructive focus:bg-destructive/10 focus:text-destructive cursor-pointer"
                            >
              ...
              <Button 
                ...
                variant="outline" 
                className="w-28 h-16 shrink-0 border-2 border-dashed border-border bg-black hover:bg-black/60 hover:border-primary/50 flex flex-col items-center justify-center gap-1 rounded-lg transition-all group"
                onClick={handleAddPage}
              >
                <Plus className="w-4 h-4 text-muted-foreground/50 group-hover:text-primary group-hover:scale-110 transition-all" />
                <span className="text-[7px] font-black text-muted-foreground/50 uppercase tracking-widest group-hover:text-primary">New Page</span>
              </Button>
            </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden">
          {notification && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 animate-in fade-in zoom-in slide-in-from-top-4 duration-500">
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-popover/90 backdrop-blur-xl border border-border shadow-2xl">
                <span className="text-lg">🥳</span>
                <span className="text-xs font-bold tracking-wider text-foreground uppercase">{notification.title}</span>
              </div>
            </div>
          )}
          ...
          <div className="w-full bg-black border-t border-border px-6 py-4 flex items-center justify-center gap-4 shrink-0 z-30">
            {isAnimating ? (
              <Button 
                aria-label="Stop Animation" 
                className="min-w-[100px] h-10 font-bold text-[11px] gap-2 bg-destructive/20 hover:bg-destructive/30 border border-destructive/30 text-destructive transition-all" 
              ...
              <Button 
                ...
                className="min-w-[100px] h-10 font-bold text-[11px] gap-2 bg-black hover:bg-muted border border-border text-foreground transition-all" 
            ...
            <Button 
              ...
              variant="outline" 
              className="h-10 px-6 text-[11px] font-bold border-border bg-transparent hover:bg-muted gap-2 text-foreground" 
            ...
            <Button 
              ...
              className="min-w-[120px] h-10 text-[11px] font-bold bg-primary hover:bg-primary/90 text-primary-foreground border-none gap-2 shadow-[0_0_15px_rgba(0,255,157,0.4)]" 
        ...
        <footer className="h-6 flex items-center justify-between px-4 text-[8px] font-bold text-muted-foreground uppercase tracking-[1.5px] bg-black border-t border-border shrink-0">
          <div className="flex gap-4">
            <span className="flex items-center gap-2 text-primary">
              <div className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_10px_rgba(0,255,157,0.8)] recording-pulse" />
              BARS: {candles.length}
            </span>
            <span className="flex items-center gap-2 hidden xs:flex">
              <div className="w-1 h-1 rounded-full bg-primary/40" /> 
              ACTIVE_ENGINE: PRO_V3_ULTRA
            </span>
          </div>
          <div className="flex items-center gap-3">
            <Monitor className="w-3 h-3 text-primary/60" />
            <span className="hidden sm:inline text-primary/40">Core 4K Precision Active</span>
          </div>
        </footer>
      </main>

      <aside className="hidden lg:flex flex-row flex-shrink-0 bg-card border-l border-border z-30" style={{ width: `${layersPanelWidth}px` }}>
        <div aria-label="Resize Layers Panel" className="w-1.5 h-full cursor-col-resize hover:bg-primary/30 transition-colors z-50 bg-border" onPointerDown={handleResizeStart} onPointerMove={handleResizeMove} onPointerUp={handleResizeEnd} />
